...
class Rescue extends React.Component {
  ...
  renderImport() {
    if (this.state.walletInfo.addresses.LNS && this.state.walletInfo.addresses.LNS.charAt(0) === "3") {
      ...
      return (
        <ButtonDisabled width="130px" fontSize={"0.8rem"}>
          IMPORTAR
        </ButtonDisabled>
      );
    }
  ...
  renderMsg() {
    if (this.state.notification && this.state.notification === "Success") {
      return (
        <H1 fontSize={"2.2rem"} margin={"2rem 0 0 0"} offSide clNormalGreen>
          {" "}
      ...
      return (
        <H1 fontSize={"2.2rem"} margin={"2rem 0 0 0"} offSide clNormalRed>
          {" "}
    ...
    return (
      <Content>
        ...
        <Input
          ...
          placeholder="Ex: fantasy deliver afford disorder primary protect garbage they defense paddle alert reveal various just dish"
        />
        <Row>
